...
class Tunjangan_jabatan extends RS_Controller
{

    public function index()
    {
        $header = $this->sendHeader('nav_tunjangan_jabatan', 'Tunjangan Jabatan');
        $header['js'] = array('maskmoney/jquery.maskMoney.min.js');
        $body = [
            'tbody' => $this->list_()
        ];
        $data = array_merge($header, $body);
        $this->template->view('VTunjangan_jabatan', $data);
    }
    ...
    private function list_()
    {
        ...
        $data = $this->MCore->list_data('payroll_tunjangan_jabatan');
        ...
        foreach ($data->result_array() as $value) {
            ...
            $button = '';
            if ($value['tj_status'] == 0) {
                $status = '<span class="badge badge-danger">Tidak Aktif</span>';
                $button .= '
                <button id="btn-aktif" data-id="' . $value['tj_id'] . '" type="button" data-toggle="tooltip" title="" class="btn btn-link btn-simple-danger" data-original-title="Aktifkan">
                    <i class="fa fa-check text-success"></i>
                </button>';
            } else {
                $button .= '
                <button id="btn-edit" type="button" data-toggle="tooltip" data-id="' . $value['tj_id'] . '" title="" class="btn btn-link btn-simple-primary btn-lg" data-original-title="Edit">
            <i class="fa fa-edit"></i></button>
                <button id="btn-nonaktif" data-id="' . $value['tj_id'] . '" type="button" data-toggle="tooltip" title="" class="btn btn-link btn-simple-danger" data-original-title="Nonaktifkan">
...
?>
            <tr class="mb-2">
                ...
                <td><?= $value['tj_nama'] ?></td>
                <td>
                    <span>0 - 16 Thn : <br> <b><?= rupiah($value['tj_baru']) ?></b></span>
                    <div class="py-1" style="border-top:1px solid #dddddd;"> </div>
                    <span>> 16 TH : <br> <b><?= rupiah($value['tj_lama']) ?></b></span>
                </td>
...
<?php
    ...
    {
        $data = $this->MCore->get_data('payroll_tunjangan_jabatan', 'tj_id = ' . $id);
    ...
    public function save()
    {
        ...
        if ($id == '') {
            if ($this->MCore->get_data('payroll_tunjangan_jabatan', array('LOWER(tj_nama)' => strtolower($data['tj_nama'])))->num_rows() > 0) {
                $arr['status'] = 0;
            ...
            $data['tj_id'] = $this->MCore->get_newid('payroll_tunjangan_jabatan', 'tj_id');
            $data['tj_created_at'] = $date;
            $sql = $this->MCore->save_data('payroll_tunjangan_jabatan', $data);
        } else {

            $data['tj_updated_at'] = $date;
            $sql = $this->MCore->save_data('payroll_tunjangan_jabatan', $data, true, array('tj_id' => $id));
        }
        $arr['status'] = $sql;
        if ($sql) {
    ...
    public function export_excel()
    {
        ...
        $listInfo = $this->MCore->list_data('payroll_tunjangan_jabatan')->result();
        ...
        $sheet->SetCellValue('C5', 'Nama Jabatan');
        ...
        foreach ($listInfo as $list) {
            ...
            $sheet->SetCellValue('C' . $rowCount, $list->tj_nama);
            $sheet->SetCellValue('D' . $rowCount, rupiah($list->tj_baru));
            $sheet->SetCellValue('E' . $rowCount, rupiah($list->tj_lama));
    ...
    {
        $sql = $this->MCore->save_data('payroll_tunjangan_jabatan', array('tj_status' => 1), true, array('tj_id' => $id));
    ...
    {
        $sql = $this->MCore->save_data('payroll_tunjangan_jabatan', array('tj_status' => 0), true, array('tj_id' => $id));
