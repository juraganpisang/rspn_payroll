...
<script>
    $(document).ready(function() {
        ...
        $('#btn-cancel').click(function() {
            ...
            $('#form-data').find('#keterangan').summernote('reset');
        });
        $('#btn-cancel').click();
        $('#search').select2({
            width: '100%',
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url('Catatan/edit') ?>/' + b.data('id'),
            dataType: 'JSON',
            async: false,
            done: function(r) {},
            beforeSend: function() {
            ...
            success: function(r) {
                ...
                $('#form-data').find('#keterangan').summernote('code', r.catatan + '<br>');
            },
            error: function(e) {
                sweetMsg('error', 'Terjadi kesalahan!');
                i.removeClass().addClass(cls);
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'GET',
                    url: '<?= base_url('Catatan/hapus') ?>/' + btn.data('id'),
                    dataType: 'JSON',
                    done: function(r) {},
                    beforeSend: function() {
                        i.removeClass().addClass('fa fa-spin fa-spinner');
