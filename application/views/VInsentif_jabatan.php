...
<div class="main-panel">
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= base_url('Insentif_jabatan/save') ?>',
                cache: false,
                contentType: false,
                processData: false,
                data: dt,
            ...
            $.ajax({
                type: 'POST',
                url: '<?= base_url('Insentif_jabatan/edit') ?>/' + b.data('id'),
                dataType: 'JSON',
                async: false,
                done: function(r) {},
                beforeSend: function() {
                ...
                success: function(r) {
                    baru = parseInt(r.ti_nominal).formatMoney(0, '.', ',');
                    currencyFormat();
                    $('#form-data').find('#id').val(r.ti_id);
                    $('#form-data').find('#nama').val(r.ti_nama);
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: '<?= base_url('Insentif_jabatan/aktif') ?>/' + b.data('id'),
                        dataType: 'JSON',
                        // async: false,
                        done: function(r) {},
                        beforeSend: function() {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: '<?= base_url('Insentif_jabatan/nonaktif') ?>/' + b.data('id'),
                        dataType: 'JSON',
                        // async: false,
                        done: function(r) {},
                        beforeSend: function() {
            ...
            e.preventDefault();
            window.open('<?php echo base_url('Insentif_jabatan/export_excel/') ?>', '_blank');
        });
